...
export function getAllShoes() {
  return function (dispatch) {
    axios("http://backend:3000/api/shoes")
...
export function getShoeById(id) {
  return function (dispatch) {
    axios(`http://backend:3000/api/shoes/id/${id}`)
      .then(({ data }) => {
...
export const deleteShoe = (shoeId) => async (dispatch) => {
    try {
      ...
      await axios.delete(`http://backend:3000/api/shoes/${shoeId}`);
      dispatch({
        type: DELETE_SHOE,
        payload: shoeId
      });
...
export const updateUserBanStatus = (userId, banStatus) => async dispatch => {
  try {
    const response = await axios.put(`http://backend:3000/users/${userId}`, { ban: banStatus });
    dispatch({
...
export const deleteUser = (userId) => async (dispatch) => {
  try {
    ...
    await axios.delete(`http://backend:3000/users/${userId}`);
...
export const userInfoChange = (userId, updatedData) => async(dispatch) =>{
  ...
  try {
    const response = await axios.put(`http://backend:3000/users/${userId}`, updatedData)
...
export const addAddress = (address) =>  async (dispatch) =>{
  try {
    const response = await axios.post(`http://backend:3000/api/adresses/`, address)
...
export const deleteAddress = (indices) => async(dispatch) => {
  try {
    ...
    const response = await axios.delete(`http://backend:3000/api/useraddresses/${addressId}`)
    console.log(response)
    const {data} = await axios(`http://backend:3000/api/users/${userId}`)
    console.log(data)
...
export const editAddress = (indices) => async(dispatch) => {
  ...
  try {  
    const {addressId, userId} = indices
    const response = await axios.put(`http://backend:3000/api/useraddresses/${addressId}`)
    console.log(response)
    const {data} = await axios(`http://backend:3000/api/users/${userId}`)
    console.log(data)
